document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateMqttStatus(connected) {
        ...
        mqttStatusText.textContent = connected ? "Po³¹czono" : "Roz³¹czono";
    ...
    function connectMQTT() {
        if (client && client.connected) {
            appendMqttLog("MQTT", "Ju¿ po³¹czono z brokerem.");
            return;
        }
        ...
        appendMqttLog("MQTT", `Próba po³¹czenia z brokerem: ${mqttUrl}`);
        ...
        client.on('connect', () => {
            ...
            client.subscribe(MQTT_TOPICS.state, (err) => {
                if (!err) {
                    ...
                    appendMqttLog("MQTT ERROR", `B³¹d subskrypcji: ${err.message}`, 'error');
                }
            });
        });
        ...
        client.on('message', (topic, message) => {
            ...
            if (topic === MQTT_TOPICS.state) {
                try {
                    ...
                    appendMqttLog("JSON PARSE ERROR", `B³¹d parsowania: ${e.message}`, 'error');
        ...
        client.on('error', (err) => {
            appendMqttLog("MQTT ERROR", `B³¹d po³¹czenia: ${err.message}`, 'error');
        ...
        client.on('close', () => {
            updateMqttStatus(false);
            appendMqttLog("MQTT", "Po³¹czenie zamkniête.");
            client = null;
        });
    }
    ...
    function disconnectMQTT() {
        if (client) {
            client.end();
            appendMqttLog("MQTT", "Roz³¹czanie od brokera...");
        } else {
            appendMqttLog("MQTT", "Klient nie jest po³¹czony.");
        }
    }
    ...
    function publishMqttMessage(topic, message) {
        if (isMqttConnected && client) {
            client.publish(topic, JSON.stringify(message), {}, (err) => {
                if (err) {
                    appendMqttLog("PUBLISH ERROR", `B³¹d publikacji: ${err.message}`, 'error');
                } else {
                    appendMqttLog(topic, JSON.stringify(message), 'sent');
                }
            });
        } else {
            appendMqttLog("MQTT ERROR", "Brak po³¹czenia MQTT!", 'error');
        }
    }
    ...
    function updateUI(data) {
        ...
        // Aktualizacja statusu p³omienia
        if (flameIndicator) {
        ...
        // Aktualizacja wydajnoœci
        if (efficiencyFill) {
            efficiencyFill.style.width = `${data.efficiency || 0}%`;
            efficiencyLabel.textContent = `${data.efficiency || 0}% Wydajnoœæ`;
        }
    ...
    function setupEventListeners() {
        ...
        // Prze³¹cznik trybu manualnego dmuchawy
        ...
        resetHistoryBtn.addEventListener('click', () => {
            if (resetPasswordInput.value === mqttPasswordInput.value) {
                ...
                resetMessageDiv.textContent = 'Nieprawid³owe has³o!';
                setTimeout(() => resetMessageDiv.textContent = '', 3000);
            }
        });
        ...
        // Prze³¹czniki wykresów temperatury
